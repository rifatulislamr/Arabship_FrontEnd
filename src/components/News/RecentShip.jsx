...
const RecentShip = () => {
  return (
    <div className='mx-4 lg:mx-40 lg:mb-40 mb-20 mt-10 z-10'>
      <div className='grid gap-10 lg:gap-6 grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3'>
        {cardData.map((card) => (
          <div
            key={card.id}
            className='relative overflow-hidden rounded-lg w-4/5 lg:h-[160%] mx-auto hover:brightness-150 hover:scale-105 duration-1000 z-50'
          >
            ...
            <Link to={`/news-details/${card.id}`} className='block'>
              ...
              <div className='absolute top-0 left-0 w-full h-full bg-black bg-opacity-40 flex justify-start items-end p-5'>
                <div className='text-white text-start'>
                  <h3 className='m-0 text-xs sm:text-2xl md:text-2xl lg:text-xl'>{card.title}</h3>
                  <p className='ml-2 m-0 text-sm  md:text-lg lg:text-sm'>{card.date}</p>
